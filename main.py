...
# Persistent dedup — survives across check cycles
seen_ann = set()
# File-based dedup — survives restarts
...
{combined}"""
    ...
    try:
        ...
        if text.lower() == "null" or text == "":
            return None
        # Find JSON in response in case model adds explanation
        start = text.find("{")
        end = text.rfind("}") + 1
...
def check_announcements():
    ...
    sent = 0
    for ann in anns[:25]:
        # Strong dedup key — symbol + description (no timestamp)
