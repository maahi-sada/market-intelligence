...
ENABLE_BROKER_NEWS = os.environ.get("ENABLE_BROKER_NEWS", "0").strip().lower() in {"1", "true", "yes"}
...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            min_score=int(os.environ.get("MIN_ALERT_SCORE", "90")),
        )
...
def is_pure_alert(result: dict[str, Any]) -> bool:
    ...
    if str(result.get("sentiment", "")).upper() == "NEUTRAL":
        return False
    if str(result.get("actionability", "")).upper() != "ALERT_IMMEDIATELY":
        return False
    return True
...
Return null if the filing is routine, low impact, from a tiny/SME/penny/suspended company, or below score 90.
...
Alert only for high-impact news: broker initiations, rating changes, target revisions, earnings estimate changes,
block/bulk deals, index inclusion/exclusion, regulatory policy, and major sector news.
...
{{
  "score": 90,
  "category": "BROKER_RATING or BLOCK_DEAL or INDEX_CHANGE or POLICY or SECTOR or OTHER",
  "sentiment": "BULLISH or BEARISH or NEUTRAL",
  "broker": null,
  "rating": null,
  "target_price": 0,
...
def format_announcement_alert(company: str, symbol: str, result: dict[str, Any], ann_time: str, nse: NSEClient) -> str:
    ...
    lines = [
        f"<b>NSE FILING | SCORE {score}/100 | {esc(sentiment)}</b>",
        "--------------------------------",
