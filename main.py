    ...
    if os.path.exists(SEEN_FILE):
        try:
            with open(SEEN_FILE, "r") as f:
                data = json.load(f)
                # Keep only last 7 days to prevent file growing forever
...
# Dict of ann_id -> timestamp (epoch float)
seen_ann = load_seen()
...
def check_announcements():
    ...
    seen_changed = False
    for ann in anns[:20]:
        # Strong unique key: symbol + full desc + announcement date
        raw_id = (
            ann.get("symbol", "") + "|" +
            ann.get("desc", "") + "|" +
            ann.get("an_dt", "") + "|" +
        ...
        if ann_id in seen_ann:
            ...
            continue
        seen_ann[ann_id] = datetime.now().timestamp()
        seen_changed = True
