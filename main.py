...
def is_seen(ann_id):
    """Check if announcement ID exists in Redis."""
    return redis("EXISTS", f"{REDIS_KEY}:{ann_id}") == 1

def mark_seen(ann_id):
    """Mark announcement ID as seen with 7-day expiry."""
    redis("SET", f"{REDIS_KEY}:{ann_id}", "1", "EX", 604800)  # 604800s = 7 days
...
def test_redis():
    ...
    if result == "PONG":
        ...
        print(f"⚠️ Redis connection issue: {result}")
...
- MANAGEMENT: extract person name, designation, whether resignation or appointment, reason if mentioned
...
def check_announcements():
    ...
    junk_keywords = [
        "newspaper", "trading window", "shareholding", "loss of share",
        "voting result", "agm", "egm", "compliance", "website", "investor meet",
        "con. call", "transcript", "presentation uploaded", "closure of"
    ]

    sent = 0
    for ann in anns[:50]:
        # Strong unique key: symbol + full desc + announcement date + attachment
        raw_id = (
            ann.get("symbol", "") + "|" +
            ann.get("desc", "") + "|" +
            ann.get("an_dt", "") + "|" +
        ...
        ann_id = hashlib.md5(raw_id.encode()).hexdigest()

        if is_seen(ann_id):
            print(f"  DUPE SKIP | {ann.get('desc','')[:50]}")
            continue
        # Mark seen immediately so crashes don't cause re-alerts on restart
        mark_seen(ann_id)
